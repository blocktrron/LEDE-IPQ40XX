...
if [ ! -n "$board" -o ! -n "$version" -o ! -r "$kernel" -o ! -r "$rootfs" -o ! "$outfile" ]; then
...
rootfs_chksum=$(checksum_file "$rootfs" "$rootfs_len")
...
#  4 bytes:  length of the contained rootfs image file (big endian)
append_be32 "$rootfs_len" "$rootfs_header_file"
#  32 bytes:  Firmware Version string (NUL terminated, 0xff padded)
...
dd if="$tmp_board_file" bs=2048 conv=sync of="$tmp_board_file-2k" &> /dev/null
